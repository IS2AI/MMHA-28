...
# --- VALIDATION ---
val_pipeline = [
    dict(type='SampleFrames', clip_len=1, frame_interval=4, num_clips=16, test_mode=True),
    dict(type='CustomRawFrameDecode', io_backend='disk'),
    dict(type='Resize', scale=(-1, 256)),
...
model = dict(
    type='Recognizer2D',
    cls_head=dict(num_classes=num_classes, num_segments=16),
    # … (backbone, head, etc.) …
    ...
    data_preprocessor=dict(
        ...
        mean=None,
        std=None,
        to_rgb=False,        # your images are already RGB
        to_float32=True,
        format_shape='NCHW'),
    train_cfg=None,
    test_cfg=None,
