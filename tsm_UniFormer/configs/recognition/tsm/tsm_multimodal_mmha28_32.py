...
data_root = '../dataset'
ann_file_train = f'{data_root}/train_all_modalities_cnn.txt'
ann_file_val   = f'{data_root}/val_all_modalities_cnn.txt'
ann_file_test  = f'{data_root}/test_all_modalities_cnn.txt'
...
# --- TRAIN ---
train_pipeline = [
    dict(type='SampleFrames', clip_len=1, frame_interval=4, num_clips=32),
...
# --- VALIDATION ---
val_pipeline = [
    dict(type='SampleFrames', clip_len=1, frame_interval=4, num_clips=32, test_mode=True),
    dict(type='CustomRawFrameDecode', io_backend='disk'),
    dict(type='Resize', scale=(-1, 256)),
...
model = dict(
    type='Recognizer2D',
    cls_head=dict(num_classes=num_classes, num_segments=32),
    # … (backbone, head, etc.) …
    # override the default ImageNet normalizer entirely:
