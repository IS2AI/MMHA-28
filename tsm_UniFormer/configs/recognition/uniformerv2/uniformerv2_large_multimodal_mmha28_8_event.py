# configs/uniformerv2_large_multimodal_mmha28_8_event.py
#
# Event UniFormerV2-Large for the MMHA-28 dataset.
# Trains and validates on Event data only, but tests on all modalities.
# Note: Skip MultiScaleCrop and Flip for event streams (not suitable for event data)
...
# ---------------------------------------------------------------------- #
#  Custom Python modules (Event for train/val, multimodal for test)
# ---------------------------------------------------------------------- #
custom_imports = dict(
    imports=[
        'mmaction.datasets.custom_event_dataset',
        'mmaction.datasets.custom_multimodal_dataset',
        'mmaction.datasets.transforms.custom_loading',
        'mmaction.datasets.transforms.normalize_event',
...
# ---------------------------------------------------------------------- #
#  Model settings (UniFormerV2-Large, 8 frames)
# ---------------------------------------------------------------------- #
num_frames = 8
num_classes = 28
...
# ---------------------------------------------------------------------- #
dataset_type = 'CustomEventDataset'  # For training and validation
dataset_type_test = 'CustomMultimodalDataset'  # For testing (all modalities)
...
ann_file_test  = f'../../cls/dataset/test_all_modalities_cnn.txt'
...
train_pipeline = [
    ...
    # Skip MultiScaleCrop for event streams
    dict(type='Resize', scale=(224, 224), keep_ratio=False),
    # Skip Flip for event streams
    dict(type='FormatShape', input_format='NCTHW'),
    dict(type='NormalizeEvent'),
...
test_evaluator = [dict(type='AccMetric'), dict(type='PerModalityAccuracy')]
